	...
	const DXGI_SURFACE_DESC* backBufferSurfaceDesc, void* pUserContext)
{
	...
	//staging textures
	{
		...
		SCG->depthStencilT->GetDesc(&Desc);
		Desc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
		Desc.Usage = D3D11_USAGE_STAGING;
		Desc.BindFlags = 0;
		hr = device->CreateTexture2D(&Desc, NULL, SCG->depthStencilTStaging.ReleaseAndGetAddressOf());
	}
	{
		D3D11_TEXTURE2D_DESC Desc;
		SCG->colorT->GetDesc(&Desc);
		Desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		Desc.Usage = D3D11_USAGE_STAGING;
		Desc.BindFlags = 0;
		hr = device->CreateTexture2D(&Desc, NULL, SCG->colorTStaging.ReleaseAndGetAddressOf());
	}
	return S_OK;
}
//--------------------------------------------------------------------------------------
